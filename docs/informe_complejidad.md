...
![formula]([https://latex.codecogs.com/svg.image?$$ \color{white} CRF_{\Pi}=\sum_{i=0}^{n-1}p_i\cdot\max(0,\,(t_i&plus;tr_i)-ts_i)$$](https://latex.codecogs.com/svg.image?$$CRF_{\Pi}=\sum_{i=0}^{n-1}p_i\cdot\max(0,\,(t_i&plus;tr_i)-ts_i)$${\color{white}[$CRF_{\Pi}=\sum_{i=0}^{n-1}p_i\cdot\max(0,(t_i&plus;tr_i)-ts_i)$]}))
